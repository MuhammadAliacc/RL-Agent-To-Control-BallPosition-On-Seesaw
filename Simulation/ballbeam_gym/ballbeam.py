...
class BallBeam():
    ...
    def __init__(self, timestep=0.05, beam_length=1.0, max_angle=0.2, init_velocity=0.0):
        ...
        self.r = 0.01975                       # ball radius    meter
        self.kg = 0.035
        self.L = beam_length                # beam length
        #self.I = 2/5*self.r**2              # solid ball inertia (omits mass)
        self.I = 2/5*(self.kg)*(self.r**2)              # solid ball inertia (omits mass)
    ...
    def reset(self):
        ...
        # lower_limit = -0.5
        # upper_limit = 0.5
        # possible_values = [round(lower_limit + i * step, 1) for i in range(int((upper_limit - lower_limit) / step) + 1)]
        # self.x = random.choice(possible_values)

        radius = self.L/2                       # beam radius
        self.t = 0.0                            # time
        #self.x = 0.4                            # ball position x
        self.x = random.choice(np.arange(-1, 1, 0.1))
    ...
    def _init_render(self, setpoint, mode):
        ...
        if mode == 'human':
            ...
            # Get the Tkinter window from the canvas and set the title
            canvas = fig.canvas
            if hasattr(canvas, 'get_tk_widget'):
                tk_widget = canvas.get_tk_widget()
                tk_widget.winfo_toplevel().title('Beam & Ball')  # Set window title here

            ax.set(xlim=(-2 * radius, 2 * radius), ylim=(-self.L / 2, self.L / 2))

            # draw ball
            self.ball_plot = Circle((self.x, self.y), self.r*3)
            ...
            _ = ax.patches[0].set_color('black')

            # draw beam
            _ = ax.plot([-cos(self.theta) * radius, cos(self.theta) * radius],
                        [-sin(self.theta) * radius, sin(self.theta) * radius], lw=4, color='silver')
    ...
    def render(self, setpoint=None, mode='human'):
        ...
        if mode == 'human':
            # update ball
            self.ball_plot.set_center((self.x, self.y+0.04))
            # update beam
            ...
            # Add velocity, position, angle, and reward as text
            if hasattr(self, 'info_text'):
                self.info_text.remove()  # Remove old text before adding new
            self.info_text = self.ax.text(
                -1.8, 0.3,  # Position on the plot
                f"Velocity: {self.v:.2f}\n"
                f"Position: ({self.x:.2f}, {self.y:.2f})\n"
                f"Angle: {self.theta:.2f} rad\n",
                #f"Reward: {reward:.2f}",
                fontsize=10,
                color='blue',
